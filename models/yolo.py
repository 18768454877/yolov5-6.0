...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            m.anchors /= m.stride.view(-1, 1, 1)  # anchors对应原图的像素大小，此处转换为在每一个特征层上的大小
            check_anchor_order(m)  # 检验anchors顺序是否正确
...
def parse_model(d, ch):  # model_dict:yolov5s.yaml, input_channels(3):[3]
    ...
    layers, save, c2 = [], [], ch[-1]  # layers, savelist(那些层需要保存), ch out=3(输出通道数)  每一层都会有一个c1、c2来表示输入和输出的通道数
    # 这个地方开始遍历backbone和head的每一层
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from:-1, number=1, module:'Conv', args:[64,6,2,2]
        m = eval(m) if isinstance(m, str) else m  # eval strings m=<class 'models.common.Conv'> 这个eval非常神奇
        for j, a in enumerate(args):
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings  if a=str a=>int args=[64, 6, 2, 2]
            except NameError:
                pass

        n = n_ = max(round(n * gd), 1) if n > 1 else n  # depth gain
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost]:  # 判断这一层是什么结构 这里都是卷积
            c1, c2 = ch[f], args[0]  # input、output channel numbers  ch=[3] f=-1 maybe & a number => c1=3、c2=64
            if c2 != no:  # if not output 如果c2不等于最终输出的255，则与gw相乘得到真实的输出通道数
                c2 = make_divisible(c2 * gw, 8)  # 还会判断是否是8的倍数，如果部不是，则变为8的倍数
        ...
        if i == 0:
            ch = []
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save)  # save: [6, 4, 14, 10, 17, 20, 23] sorted-> [4, 6, 10, 14, 17, 20, 23]
