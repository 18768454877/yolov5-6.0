...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, ?groups
        ...
        # 可以自定义act操作，但必须是nn.Module内的模块，默认是使用SiLU，如果都不是则使用nn.Identity()，即为不做操作
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        # bottleNeck shortcut = 1 有 残差 连接 , 2 没有
...
class SimSPPF(nn.Module):
    ...
    美团YOLOv6提出的模块
    看起来和SPPF的差别只在于激活函数用的是ReLU
    简单测试，单个ConvBNReLU速度要比ConvBNSiLU快18%'''
    def __init__(self, in_channels, out_channels, kernel_size=5):
        super().__init__()
        c_ = in_channels // 2  # hidden channels
        self.cv1 = Conv(in_channels, c_, 1, 1, act=nn.ReLU)
        self.cv2 = Conv(c_ * 4, out_channels, 1, 1, act=nn.ReLU)
        self.m = nn.MaxPool2d(kernel_size=kernel_size, stride=1, padding=kernel_size // 2)

    def forward(self, x):
        x = self.cv1(x)
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')   # 抑制特定的警告消息，以防止它们干扰代码的执行或输出
            y1 = self.m(x)
            y2 = self.m(y1)
            return self.cv2(torch.cat([x, y1, y2, self.m(y2)], 1))
